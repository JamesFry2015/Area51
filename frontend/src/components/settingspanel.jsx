...
const SettingsPanel = ({
  ...
  settings,
  onSettingChange,
  attachmentSettings, // NEW Prop
  onAttachmentSettingChange, // NEW Prop
  generationSettings,
  ...
  const handleSaveApiConfig = async (configData) => {
    try {
      ...
      // NEW: Check if it was active for attachments too
      const wasAttachmentActive = configToEdit && attachmentSettings &&
                        attachmentSettings.model === configToEdit.model &&
                        attachmentSettings.baseUrl === configToEdit.proxy_url;
      ...
      // Update attachment settings if we just edited the active attachment config
      if (wasAttachmentActive) {
        onAttachmentSettingChange({
            model: configData.model,
            baseUrl: configData.proxy_url,
            apiKey: configData.api_key || '',
            requestBody: configData.request_body || null
  ...
  // Helper to check which config is active
  const isConfigActive = (config, currentSettings) => {
    return currentSettings && 
           currentSettings.model === config.model && 
           currentSettings.baseUrl === config.proxy_url;
  };

  const handleSelectMain = (config) => {
    onSettingChange({
      model: config.model,
  ...
  // NEW: Handler for selecting attachment model
  const handleSelectAttachment = (config) => {
    onAttachmentSettingChange({
      model: config.model,
      baseUrl: config.proxy_url,
      apiKey: config.api_key || '',
      ...
      <div className="settings-content">
        ...
        <div className="api-config-list">
          {apiConfigs.length > 0 ? apiConfigs.map(config => {
            const isMain = isConfigActive(config, settings);
            const isAttach = isConfigActive(config, attachmentSettings);

            return (
              <div key={config.id} className={`config-item ${isMain ? 'active' : ''}`}>
                <div className="config-item-header">
                    <h4>{config.name}</h4>
                    <div className="badges">
                        {isMain && <span className="active-badge">Chat Model</span>}
                        {isAttach && <span className="attachment-badge">👁️ Vision</span>}
                    </div>
                    <div className="config-item-actions"><button onClick={() => handleEditClick(config)}>Edit</button><button onClick={() => handleDeleteClick(config.id)}>Delete</button></div>
                </div>
                <div className="config-item-details"><span>{config.model}</span><span>{config.proxy_url}</span></div>
                
                <div className="selection-buttons">
                    {!isMain && <button className="select-btn" onClick={() => handleSelectMain(config)}>Use for Chat</button>}
                    {!isAttach && <button className="select-btn secondary" onClick={() => handleSelectAttachment(config)}>Use for Attachments</button>}
                </div>
              </div>
            )
...
// ... Rest of the sub-components (SystemAndMemoryForm, GenerationSettingsForm, SliderInput, ConfigForm) remain unchanged ...
